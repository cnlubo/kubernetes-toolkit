...
do
    ...
    ssh ${k8s_user:?}@${node_ip} "docker pull ${my_registry}/k8s-gcr-io-metrics-server-amd64:v0.3.3"
    #ssh ${k8s_user:?}@${node_ip} "docker pull ${my_registry}/k8s-gcr-io-addon-resizer:1.8.5"
    ssh ${k8s_user:?}@${node_ip} "docker tag ${my_registry}/k8s-gcr-io-metrics-server-amd64:v0.3.3 \
    ...
    ssh ${k8s_user:?}@${node_ip} "docker rmi -f ${my_registry}/k8s-gcr-io-metrics-server-amd64:v0.3.3"
